...
We define three properties for a secure DAO voting system:
\begin{enumerate}
    ...
    \item Vote Finality: Once the interaction between the member and the DAO contract is finished, the vote count should not revert or otherwise be changed by the actions of another member.
\end{enumerate}

Properties 1 and 2 are collectively bundled as part of the \textit{Vote Validated} DAO contract event. Property 3 is covered in our model by the terminating path of execution and lack of looping. Each call of the vote function is treated as a separate execution. 
